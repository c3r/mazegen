...
constexpr int k_PixelSize = 3;
constexpr int k_ScreenX = 10;
constexpr int k_ScreenY = 25;

constexpr SDL_Color k_NotVisitedColor = { 0x99, 0x33, 0x66, 0xFF };
constexpr SDL_Color k_WallColor = { 0x33, 0x33, 0x33, 0xFF };
constexpr SDL_Color k_VisitedColor = { 0xFF, 0xCC, 0x00, 0xFF };
...
void DrawCell(int x, int y, int walls, int cell_width, int cell_height, int wall_size, bool visited) 
{
  ...
  DrawRect(x, y, cell_width, cell_height, visited ? k_VisitedColor : k_NotVisitedColor);
  ...
  if (walls & S_WALL)
    DrawRect(x, y + cell_height, cell_width + wall_size, wall_size, k_WallColor);
  else
